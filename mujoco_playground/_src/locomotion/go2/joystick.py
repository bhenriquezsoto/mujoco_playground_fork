...
class Joystick(go2_base.Go2Env):
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    print("Stepping environment", flush=True)
    ...
    # print("Getting observations", flush=True)
    obs = self._get_obs(data, state.info)
    done = self._get_termination(data)

    # print(f"""Printing detailed observations:""", flush=True)
    # print("Printing rewards")
    rewards = self._get_reward(
        data, action, state.info, state.metrics, done, first_contact, contact
    )
    # jax.debug.print("Rewards from get_reward: {rewards}", rewards=rewards)

    rewards = {
        k: v * self._config.reward_config.scales[k] for k, v in rewards.items()
    }
    # jax.debug.print("Rewards after scaling: {rewards_after_scaling}", rewards_after_scaling=rewards)

    reward = jp.clip(sum(rewards.values()) * self.dt, 0.0, 10000.0)
    # jax.debug.print("Rewards sum total after clipping: {rewards_clipped}", rewards_clipped=reward)
    # print("Self.dt: ", self.dt, flush=True)
    ...
    # jax.debug.print("Noisy Linvel: {linvel}", linvel=obs["state"][:3])
    # jax.debug.print("Noisy Gyro: {gyro}", gyro=obs["state"][3:6])
    # jax.debug.print("Noisy Gravity: {gravity}", gravity=obs["state"][6:9])
    # jax.debug.print("Noisy Joint Angles: {joint_angles}",
    #                 joint_angles=obs["state"][9:21])
    # jax.debug.print("Noisy Joint Velocities: {joint_vel}",
    #                 joint_vel=obs["state"][21:33])
    # jax.debug.print("Last Action: {last_act}", last_act=obs["state"][33:45])
    # jax.debug.print("Command: {command}", command=obs["state"][45:48])
    state.info["steps_until_next_cmd"] = jp.where(
        done | (state.info["steps_until_next_cmd"] <= 0),
        jp.round(jax.random.exponential(key2) * 5.0 / self.dt).astype(jp.int32),
        state.info["steps_until_next_cmd"],
    )
    # print(f'{state.info["steps_until_next_cmd"]} steps until next command', flush=True)
    # jax.debug.print("{steps} steps until next command",
                    # steps=state.info["steps_until_next_cmd"])
    state.info["feet_air_time"] *= ~contact
    # print(f"Feet air time: {state.info['feet_air_time']}", flush=True)
    # jax.debug.print("Feet air time: {feet_air_time}", feet_air_time=state.info["feet_air_time"])
    state.info["last_contact"] = contact
    # print(f"Last contact: {state.info['last_contact']}", flush=True)
    # jax.debug.print("Last contact: {last_contact}", last_contact=state.info["last_contact"])
    state.info["swing_peak"] *= ~contact
    # print(f"Swing peak: {state.info['swing_peak']}", flush=True)
    # jax.debug.print("Swing peak: {swing_peak}", swing_peak=state.info["swing_peak"])
    ...
    # jax.debug.print("State reward: {state}", state=state)
    return state
  ...
  def   _get_obs(
    ...
    # jax.debug.print("Gyro {gyro}", gyro=gyro)
    ...
    # jax.debug.print("Gravity {gravity}", gravity=gravity)
    ...
    # jax.debug.print("Joint angles {joint_angles}", joint_angles=joint_angles)
    ...
    # jax.debug.print("Joint velocities {joint_vel}", joint_vel=joint_vel)
    ...
    # jax.debug.print("Linvel {linvel}", linvel=linvel)
    ...
    # jax.debug.print("Accelerometer {accelerometer}", accelerometer=accelerometer)
    angvel = self.get_global_angvel(data)
    # jax.debug.print("Angvel {angvel}", angvel=angvel)
    feet_vel = data.sensordata[self._foot_linvel_sensor_adr].ravel()
    # jax.debug.print("Feet_vel {feet_vel}", feet_vel=feet_vel)
  ...
  def _get_reward(
    ...
    # dof_pos_limits = self._cost_joint_pos_limits(data.qpos[7:])
    # return {
    #   "tracking_lin_vel": jp.zeros_like(tracking_lin_vel),
    #   "tracking_ang_vel": jp.zeros_like(tracking_ang_vel),
    #   "lin_vel_z": jp.zeros_like(lin_vel_z),
    #   "ang_vel_xy": jp.zeros_like(ang_vel_xy),
    #   "orientation": jp.zeros_like(orientation),
    #   "stand_still": jp.zeros_like(stand_still),
    #   "termination": jp.zeros_like(termination),
    #   "pose": jp.zeros_like(pose),
    #   "torques": jp.zeros_like(torques),
    #   "action_rate": jp.zeros_like(action_rate),
    #   "energy": jp.zeros_like(energy),
    #   "feet_slip": jp.zeros_like(feet_slip),
    #   "feet_clearance": jp.zeros_like(feet_clearance),
    #   "feet_height": jp.zeros_like(feet_height),
    #   "feet_air_time": jp.zeros_like(feet_air_time),
    #   "dof_pos_limits": jp.zeros_like(dof_pos_limits),
    # }
  ...
  def _cost_stand_still(
    ...
    # Penalize moving (incapabel of maintaining position) when no command is given.
  ...
  def _cost_feet_slip(
    ...
    # Penalize feet slip when in contact with the ground (seems weird,
    # as it does a very similar thing to cost_stand_still)
  ...
  def _cost_feet_clearance(self, data: mjx.Data) -> jax.Array:
    # Penalize feet that are too high.
    feet_vel = data.sensordata[self._foot_linvel_sensor_adr]
    vel_xy = feet_vel[..., :2]
  ...
  def _cost_feet_height(
    ...
    # Penalize feet that do not reach a minimum height during swing.
    cmd_norm = jp.linalg.norm(info["command"])
